...
| `canvas.png` | The graph canvas: a server node fanning out to two tools via *Expose tool*, each tool running an HTTP request and returning JSON. **Committed.** | README "The canvas" |
...
| `claude-code.png` | Claude Code's `/mcp` listing, showing a connected MCPeasy server and its tools with their read-only annotations. **Committed.** | README "Connect to Claude Code" |

Use these exact filenames — the README links to them directly.

**`test-console.png` is not in the repo yet**, so its embed is currently wrapped
in an HTML comment in the top-level README. A Markdown link to a missing file
renders as a broken-image icon on GitHub, which looks worse than no image at
all; commenting it out keeps the page clean until the file arrives. To restore
it, save the PNG here and delete the two comment marker lines that surround its
`![...]` line — the embed itself is preserved verbatim, so nothing needs
retyping.
